      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="body-sm text-text-muted-custom mt-1">
            Stay updated with all branch activity
          </p>
        </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y">
            {filtered.map((n) => (
              <div
                key={n.id}
                className={`flex items-start gap-4 p-4 cursor-pointer transition-colors ${getNotificationBg(n.type, n.read)}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className={`text-sm ${!n.read ? "font-bold text-charcoal" : "font-medium text-charcoal/80"}`}>
                      {n.title}
                    </p>
                    {!n.read && <span className="h-2 w-2 rounded-full bg-emerald shrink-0" />}
                  </div>
                  <p className="text-xs text-text-muted-custom line-clamp-2">{n.message}</p>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12 text-text-muted-custom">
                <Bell className="h-10 w-10 mx-auto mb-3 text-emerald/20" />
