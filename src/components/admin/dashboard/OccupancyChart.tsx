  ...
  return (
    <Card className="py-4 shadow-xs border-transparent">
      <CardHeader className="px-5 pb-0">
        <CardTitle className="text-sm font-semibold text-charcoal">Room Status</CardTitle>
      </CardHeader>
      <CardContent className="px-5 pt-2">
        <div className="h-[200px] w-full relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-[10px] text-text-muted-custom uppercase tracking-wider">Utilization</span>
          </div>
        </div>
        {/* Legend */}
        <div className="grid grid-cols-2 gap-2 mt-3">
          {occupancyData.map((item) => (
            <div key={item.name} className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-charcoal ml-auto">{item.value}%</span>
            </div>
          ))}
        </div>
      </CardContent>
